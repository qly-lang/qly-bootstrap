...
(test parse-integer
  ...
  (defun integer-type-test (int type)
    (let* ((i (princ-to-string int))
      ...
      (is (= int (qly-int-value i-parse)))
      (is (eql type (qly-int-type i-parse)))))

  (integer-type-test 0 :|int32|)
  (integer-type-test (1- (expt 2 31)) :|int32|)
  (integer-type-test (- (expt 2 31)) :|int32|)

  (integer-type-test (expt 2 31) :|int64|)
  (integer-type-test (1- (- (expt 2 31))) :|int64|)
  (integer-type-test (1- (expt 2 63)) :|int64|)
  (integer-type-test (- (expt 2 63)) :|int64|)

  (integer-type-test (expt 2 63) :|int128|)
  (integer-type-test (1- (- (expt 2 63))) :|int128|)
  (integer-type-test (1- (expt 2 127)) :|int128|)
  (integer-type-test (- (expt 2 127)) :|int128|)
...
(test parse-uint
  ...
  (defun uint-type-test (uint base type)
    (let* ((*print-base* base)
           (u (esrap:text "0"
                          (ecase base
                            (2 "b")
      ...
      (is (= uint (qly-uint-value u-parse)))
      (is (eql base (qly-uint-base u-parse)))
      (is (eql type (qly-uint-type u-parse)))))

  (uint-type-test 0 16 :|uint32|)
  (uint-type-test 0 8 :|uint32|)
  (uint-type-test 0 2 :|uint32|)
  (uint-type-test (1- (expt 2 32)) 16 :|uint32|)
  (uint-type-test (expt 2 32) 16 :|uint64|)
  (uint-type-test (1- (expt 2 64)) 8 :|uint64|)
  (uint-type-test (expt 2 64) 8 :|uint128|)
  (uint-type-test (1- (expt 2 128)) 2 :|uint128|)
  (uint-type-test (expt 2 128) 2 :|biguint|))
